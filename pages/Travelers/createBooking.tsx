...
const CreateBookingModal: FC<CreateBookingModalProps> = ({ showModal, setShowModal, value, roomId, dates}) => {
  ...
  const [checkoutData, setCheckoutData] = useState<string>("")
  ...
  useEffect(() => {
    if (dates) {
      setCheckinData(dates?.checkin)
      setCheckoutData(dates?.checkout)
    }
  }, [dates])

  useEffect(() => {
    if (checkinData && checkoutData) {
      const price = roomsType.find((type: IroomsType) => type.id === rooms.data.find((room: IRooms) => room.id === roomId)?.roomTypeID)?.price
      const tax = roomsType.find((type: IroomsType) => type.id === rooms.data.find((room: IRooms) => room.id === roomId)?.roomTypeID)?.tax
      const totalDays = moment(checkoutData).diff(moment(checkinData), 'days')
  ...
  const FormObserver: React.FC = () => {
    ...
    useEffect(() => {
      setCheckinData(values?.checkin)
      setCheckoutData(values?.checkout)
    }, [values]);
    return null;
  }
                  ...
                  <Form >
                    ...
                    <div className="grid px-5 pt-5 grid-cols-2 gap-4">
                      <div className="flex items-center">
                          ...
                          <p className="text-2xl ml-2 font-light">{rooms.data.find((room: IRooms) => room.id === roomId)?.name}</p>
                        </div>
                      <div className="flex items-center">
                          <p className="text-2xl font-semibold">Price:</p>
                          <p className="text-2xl ml-2 font-light">{`$${totalPrice || 0}`}</p>
                        </div>
                      <div>
